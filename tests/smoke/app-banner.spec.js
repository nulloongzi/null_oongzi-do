...
test.describe('앱 설치 배너 — Android', () => {
    ...
    test('노출 + CTA(Play Store) + 닫기 지속', async ({ page }) => {
        ...
        // CTA → Play Store 등록 페이지 (패키지 com.nulloongzi.nulloongzido)
        await expect(page.locator('.aib-cta')).toHaveAttribute(
            'href',
            /^https:\/\/play\.google\.com\/store\/apps\/details\?id=com\.nulloongzi\.nulloongzido(&|$)/
        );
...
test.describe('앱 설치 배너 — iOS', () => {
    test.use({ userAgent: IOS_UA, viewport: { width: 390, height: 844 } });

    test('iOS UA(앱 미출시)에서는 미노출', async ({ page }) => {
        await page.goto('/');
        await page.waitForTimeout(600);
        await expect(page.locator('.app-install-banner')).toHaveCount(0);
    });
