...
- 종목 구분: **6인제**(로테이션 · 대각) / **9인제**(자리 고정 3×3)
...
| id | 종목 | 인원 | 슬롯 | 설명 |
|----|------|------|------|------|
| `mb2` | v6 | 6 | S, OP, OH, OH, MB, MB | 리베로 없음 |
| `mb1li` | v6 | 6 | S, OP, OH, OH, MB, Li | 센터 1 + 리베로 1 |
| `mb2li` | v6 | 7 | S, OP, OH, OH, MB, MB, Li | 리베로가 후위 센터와 교대 |
| `v9` | v9 | 9 | FL, FC, FR, CL, CC, CR, BL, BC, BR | 전 · 중 · 후위 3자리씩 |

6인제 대각 규칙: S↔OP, OH↔OH, MB↔Li(or MB).
9인제는 로테이션이 없어 코트 아홉 칸이 곧 자리다 — 대각 규칙이 없다.
...
const POS_V9 = ['FL', 'FC', 'FR', 'CL', 'CC', 'CR', 'BL', 'BC', 'BR'];
