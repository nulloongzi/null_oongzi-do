...
- [ ] **1-3 (XSS 출력단 escape)** — 모든 `innerHTML` 사용처를 `textContent` 또는 escape 헬퍼로 통일
  - 신규 `js/dom-utils.js`에 `escapeHtml`, `sanitizeUrl`, `sanitizeInstaHandle`, `sanitizeFilename` 추가
  - 적용 파일: `js/club-detail.js`, `js/map-core.js`, `js/lunchbox.js`, `js/profile.js`
