...
<head>
    ...
    <style>
        * {{ box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Apple SD Gothic Neo", "Malgun Gothic", "맑은 고딕", sans-serif; }}
        ...
        .ft-header {{ background: #fafafa; font-weight: 800; color: #555; border-bottom: 1px solid #eee; }}
        .ft-time-col {{ background: #fafafa; color: #999; font-size: 9px; border-right: 1px solid #eee; font-weight: 500; }}
        .ft-block {{ background: var(--brand-color); opacity: 0.9; }}
...
<body>
    ...
    <script>
        ...
        // [수정된 파싱 함수] 3가지 케이스 모두 처리
        function parseScheduleText(text) {{
            var scheduleMap = {{}};
            if (!text) return scheduleMap;

            // 1. " / "으로 구분하여 세그먼트 나누기 (Format 3 대응)
            // 구분자가 없을 수도 있으므로 일단 배열로 만듦
            var segments = text.split(" / "); 
            
            segments.forEach(function(segment) {{
                // 시간 추출 (HH:MM~HH:MM 형태)
                // 정규식: 숫자:숫자 ~ 숫자:숫자
        ...
        function renderTimetables(scheduleText) {{
            ...
            days.forEach(function(day) {{
                ...
                if (data) {{
                    ...
                    bubble.innerHTML = '<div class="st-day-text">' + day + '요일</div><div class="st-time-text">' + data.text + '</div>';
            ...
            for (var h = 6; h <= 24; h++) {{
                ...
                days.forEach(day => {{
                    ...
                    var data = scheduleData[day];
                    if (data) {{
                        // 종료 시간 전까지 색칠 (예: 20~22시면 20, 21시 블록 색칠)
                        if (h >= data.startH && h < data.endH) {{
