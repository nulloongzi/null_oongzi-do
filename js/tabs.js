...
(function () {
    ...
    // 안치기(팀 짜기)는 지도 SDK를 안 쓰는 독립 페이지다. 오버레이 대신 페이지 이동으로 연다.
    // 나가기 전에 현재 탭·필터를 URL에 박아둔다. 안치기의 뒤로가기가 history.back() 이라
    // 이 URL 그대로 돌아오고, 착지 복원(app.js openPickupListLink)이 필터를 되살린다.
    // (안 박아두면 픽업에 들어오자마자 연 경우 URL이 비어 있어 동호회 탭으로 떨어진다)
    window.openAnchigi = function () {
        if (window.syncPickupUrl) window.syncPickupUrl();
        if (window.track) window.track('open_anchigi', { tab: window.currentTab });
        location.href = 'anchigi.html?lang=' + (window.currentLang || 'ko');
    };
